...
import coolalias.structuregenapi.util.BlockRotationData.Rotation;
...
public abstract class StructureGeneratorBase extends WorldGenerator
{
	...
	private final void setBlockAt(World world, int fakeID, int realID, int meta, int customData1, int customData2, int x, int y, int z)
	{
		...
		{
			if (BlockRotationData.getBlockRotationType(realID) != null) {
				int rotations = ((isOppositeAxis() ? structureFacing + 2 : structureFacing) + facing) % 4;
				meta = GenHelper.getMetadata(rotations, Math.abs(realID), meta);
			}

			if (BlockRotationData.getBlockRotationType(realID) != null &&
				(BlockRotationData.getBlockRotationType(realID) == Rotation.WALL_MOUNTED ||
				BlockRotationData.getBlockRotationType(realID) == Rotation.LEVER))
			{
	...
	private final boolean removeBlockAt(World world, int fakeID, int realID, int x, int y, int z, int rotations)
	{
		...
		else if (Math.abs(realID) == worldID || GenHelper.materialsMatch(realID, worldID)) // || Math.abs(fakeID) > 4095
		{
